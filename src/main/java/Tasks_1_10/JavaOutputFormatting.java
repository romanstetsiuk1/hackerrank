...
public class JavaOutputFormatting {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 3; i++) {
            String s1 = sc.next();
            int x = sc.nextInt();
            System.out.println(convertStringValue(s1) + convertIntValue(x));
        }
        System.out.println("================================");

    }

    public static String convertStringValue(String inputString) {
        if (inputString.length() <= 15) {
            for (int i = inputString.length(); i < 15; i++) {
                inputString += " ";
            }
        }
        return inputString;
    ...
    public static String convertIntValue(int inputInt) {
        String userValue = Integer.toString(inputInt);
